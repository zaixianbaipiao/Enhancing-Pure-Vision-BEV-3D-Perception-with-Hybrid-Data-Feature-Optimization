...
from mmcv.cnn.bricks import ContextBlock  # 添加这行
...
class SABEV(BEVDepth4D):
    ...
    def __init__(self, use_bev_paste=True, bda_aug_conf=None, **kwargs):
         # +++ 新增：从kwargs中移除context_block +++
        context_block_cfg = kwargs.pop('context_block', None)
        ...
        # +++ 直接初始化 ContextBlock（不再从配置读取） +++
        self.context_block = ContextBlock(
            in_channels=512,  # 与 img_neck 输出通道一致
            ratio=1,
            pooling_type='avg', #avg
    ...
    def image_encoder(self, img):
        ...
        if self.with_img_neck:
            ...
            # +++ 新增：应用 ContextBlock +++
            # x[1] 是颈部输出，形状为 [B*N, 512, H, W]
            # 应用 ContextBlock 增强特征
            x[1] = self.context_block(x[1])
